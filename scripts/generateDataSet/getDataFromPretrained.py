...
if __name__ == "__main__":
    ...
    path = os.path.join(current_dir, '../..', 'dexartEnv/assets/data')
    num_episodes = 1
    ...
    while index < needIter:
        ...
        for j in range(env.horizon):
            ...
            if j % 10 == 0:
                np.save(f"{path}/{task_name}/seg/{index}.npy", obs['instance_1-seg_gt'])
                np.save(f"{path}/{task_name}/pc/{index}.npy", obs['instance_1-point_cloud'])
                index += 1
